/**
 * ProjectsSection (Upcoming Events & Competitions)
  ...
  return (
    <section id="projects" className="py-16 md:py-20 relative overflow-hidden scroll-mt-28 md:scroll-mt-32">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-ndc-purple/5 blur-[120px] animate-pulse delay-700"></div>
      </div>
      ...
      <div className="section-container relative z-10">
          ...
          {upcoming.map((item, index) => (
            <div key={index} className="group relative p-7 rounded-3xl transition-all duration-500 hover:-translate-y-1 border backdrop-blur-xl bg-white/[0.04] border-white/10 hover:border-ndc-purple/40">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest bg-white/5 border border-white/10 text-gray-300">
                  {item.badge}
                </span>
              </div>
        ...
        <div className="mt-16 text-center">
          <div className="inline-flex items-center gap-3 p-6 rounded-3xl border border-dashed bg-white/5 border-white/10 text-gray-300">
            <Code2 className="w-6 h-6 text-ndc-purple animate-bounce" />
            <p className="text-lg font-medium">
              Want to participate or volunteer? <span className="text-ndc-blue hover:underline cursor-pointer">Apply via the form</span>
            </p>
          </div>
        </div>
      </div>

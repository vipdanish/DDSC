...
const Gallery = () => {
  ...
  const getImageStyles = (position: string, index: number) => {
    const baseStyles = "absolute transition-all duration-500 ease-in-out cursor-pointer";
    
    switch (position) {
      case 'center':
        return `${baseStyles} left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-96 z-30 opacity-100`;
      case 'side':
        const diff = (index - currentIndex + galleryItems.length) % galleryItems.length;
        const isRight = diff === 1;
        return `${baseStyles} ${
          isRight ? 'left-[65%]' : 'left-[15%]'
        } top-1/2 -translate-y-1/2 w-48 h-64 z-20 opacity-75`;
      default:
      ...
      <div className="section-container">
        <div className="max-w-3xl mx-auto text-center mb-16 animate-fade-in">
          <h2 className={`heading-md mb-6 transition-all duration-500 hover:scale-105 ${
            ...
            Our <span className="gradient-text animate-pulse">Gallery</span>
          </h2>
